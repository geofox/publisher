...
// past maxBlobBytes with an ancillary chunk. DecodeConfig reads only the IHDR
// (so it reports the bomb dimensions), while the >1 MB size forces fitBlob down
// the "can't safely resize" path — proving the guard fires before image.Decode
...
func craftBigPNGHeader(w, h uint32) []byte {
	...
	// tEXt padding to exceed maxBlobBytes (read after IHDR, so it doesn't affect
	// the dimensions DecodeConfig returns).
	pad := make([]byte, maxBlobBytes+1024)
