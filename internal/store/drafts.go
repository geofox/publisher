...
func insertDraftMedia(tx *sql.Tx, draftID string, media []Media) error {
	for _, m := range media {
		if _, err := tx.Exec(
			`INSERT INTO draft_media(draft_id, ordinal, blossom_url, sha256, mime, dim, blurhash, size_bytes, alt, duration_secs, poster_url)
			 VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			draftID, m.Ordinal, m.BlossomURL, m.SHA256, m.Mime, m.Dim, m.Blurhash, m.SizeBytes, m.Alt, m.DurationSecs, m.PosterURL,
		); err != nil {
...
func (s *Store) getDraftMedia(draftID string) ([]Media, error) {
	rows, err := s.sql.Query(
		`SELECT ordinal, blossom_url, sha256, COALESCE(mime,''), COALESCE(dim,''),
		        COALESCE(blurhash,''), COALESCE(size_bytes,0), COALESCE(alt,''), COALESCE(duration_secs,0), COALESCE(poster_url,'')
	...
	for rows.Next() {
		var m Media
		if err := rows.Scan(&m.Ordinal, &m.BlossomURL, &m.SHA256, &m.Mime, &m.Dim, &m.Blurhash, &m.SizeBytes, &m.Alt, &m.DurationSecs, &m.PosterURL); err != nil {
			return nil, err
		}
		out = append(out, m)
	}
