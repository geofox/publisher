...
func (s *Store) PublicFeed(limit int) ([]Post, error) {
	...
	for i := range out {
		...
		mrows, err := s.sql.Query(`SELECT ordinal, blossom_url, sha256, mime, dim, blurhash, size_bytes, alt, COALESCE(duration_secs,0), COALESCE(poster_url,'') FROM media WHERE post_id=? ORDER BY ordinal`, out[i].ID)
		if err != nil {
			return nil, err
		}
		for mrows.Next() {
